...
ARM_SWING_VEL_THRESHOLD = 0.25 # 摆臂触发的速度阈值 (m/s)
...
class ActionRobotController(BaseController):
    ...
    def __init__(self, yaml_path, node_name="action_robot_controller"):
        ...
        self.get_logger().info(f"观测维度: {self.obs_dim}, Policy关节数: {self.param['policyJointNum']}")
    ...
    def inference(self):
        ...
        self.obs[0, 6:9] = vel_cmd *(np.linalg.norm(vel_cmd)>0.15) * self.vel_cmd_scale  
    ...
    def publish_motor_commands(self):
        ...
        # 腰部电机12、13死区限制：±0.1rad内视为0（消除微小抖动）
        # if abs(self.motor_commands[12].position_des) < 0.1:
        #     self.motor_commands[12].position_des = 0.0
        # if abs(self.motor_commands[13].position_des) < 0.1:
        #     self.motor_commands[13].position_des = 0.0
...
def main(args=None):
    ...
    if not os.path.exists(yaml_path):
        yaml_path = os.path.join(
            ...
            '../../config/waist_15_robot_controller_config.yaml'
        )
