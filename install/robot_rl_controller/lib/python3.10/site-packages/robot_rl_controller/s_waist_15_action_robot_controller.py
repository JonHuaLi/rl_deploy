"""
ActionRobotController - 机器人强化学习控制器（动作版 - 15关节含腰部）
修改：订阅外部状态机（参考joy_action_right_arm），根据状态决定输出
- disable/damping/ready: 所有26个电机输出0命令
- running: 下肢12+腰部3个电机执行policy输出（15关节），上肢10个电机执行动作/默认位置

关节顺序重排序映射（通过policy_joint_indices实现）：
- Policy模型期望: legs(0-11), yaw(12), roll(13), pitch(14)
- 硬件电机实际:   legs(0-11), roll(12), pitch(13), yaw(14)
- policy_joint_indices: [0-11, 14, 12, 13] 完成双向映射
  - 输入: motor_feedback[14] → propri_obs[12] → policy看到yaw在位置12
  - 输出: actions[12](yaw) → motor_commands[14](yaw电机)
"""
...
ACTION_JSON_DIR = "/app/etc/algo/action"
ACTION_JSON_MAP = {
...
ARM_SWING_AMPLITUDE = 0.0  # 行走摆臂幅度 (rad)
ARM_SWING_VEL_THRESHOLD = 50.15  # 摆臂触发的速度阈值 (m/s)
...
class ActionRobotController(BaseController):
    """机器人强化学习控制器（动作版 - 15关节含腰部）
    
    26关节人形机器人，Policy控制前15个关节（12下肢 + 3腰部）
    观测维度：56维
    ...
    - STATE_RUNNING(3): 下肢12+腰部3电机执行policy，上肢执行动作/默认，头部保持默认
    """
    ...
    def __init__(self, yaml_path, node_name="action_robot_controller"):
        ...
        # )
        algo_dir = self.param.get('algo_config_dir')
        if algo_dir:
            onnx_path = os.path.join(algo_dir, 'onnx', self.param["onnx_path"])
        else:
            onnx_path = os.path.join(
                get_package_share_directory('robot_rl_controller'),
                'onnx',
        ...
        self.robs_dim = self.param["r_observation_dim"]
        self.robs = np.zeros([1, self.robs_dim], dtype=np.float32)        
        ...
        self.get_logger().info("Action Robot Controller (15关节含腰部) 已启动")
        self.get_logger().info("状态机模式：订阅外部 /system/lrs_cmd 控制")
        self.get_logger().info("disable/damping -> 所有电机输出0")
        self.get_logger().info("ready -> 全部26电机3秒缓慢移动到readyPos")
        self.get_logger().info("running -> 下肢12+腰部3电机执行policy(15关节)，上肢执行动作/默认，头部保持默认")
        self.get_logger().info(f"Policy关节映射(policy→硬件): {list(enumerate(self.policy_joint_indices))}")
        self.get_logger().info(f"观测维度: {self.robs_dim}, Policy关节数: {self.param['policyJointNum']}")
        ...
        if self.external_system_state == STATE_RUNNING:
            if self.external_system_mode == MODE_DANCE:
                if self.current_dance_action == 10:
                    # DANCE_1：使用推理输出占位，保持站立（后续替换为实际舞蹈policy）
    ...
    def show_log(self):
        ...
        # 关节位置信息（显示完整的15个关节: 12腿部 + 3腰部）
        dof_policy_str = ", ".join([f"{pos:.2f}" for pos in self.propri_obs.joint_pos])
        log_dof_policy = ["dof_policy(15)", f"[{dof_policy_str}]"]
        
        # 计算绝对位置（显示完整的15个关节）
        # 注意: defaultPos是按硬件电机索引排列的，这里取前policyJointNum个
        # 由于propri_obs已通过policy_joint_indices重排序，需要用对应电机的defaultPos
        dof_urdf_parts = []
        for i, idx in enumerate(self.policy_joint_indices):
            if i < self.param["policyJointNum"]:
                dof_urdf_parts.append(self.propri_obs.joint_pos[i] + self.param["defaultPos"][idx])
        dof_urdf_str = ", ".join([f"{pos:.2f}" for pos in dof_urdf_parts])
        log_dof_urdf = ["dof_urdf(15)", f"[{dof_urdf_str}]"]
        
        # Action信息（显示完整的15个关节）
        action_str = ", ".join([f"{act:.2f}" for act in self.actions])
        log_action = ["action(15)", f"[{action_str}]"]
        
        # 观测向量信息
        log_obs = ["robservation_dim", f"{self.robs_dim}"]
    ...
    def inference(self):
        ...
        self.vel_cmd_scale = np.array(self.param['vel_cmd_scale'], dtype=np.float32)
        self.obs[0, 6:9] = vel_cmd *(np.linalg.norm(vel_cmd)>0.20) * self.vel_cmd_scale
        # 9:24 - joint_pos (15维: 12腿部 + 3腰部)
        # propri_obs.joint_pos 已通过 base_controller 的 low_state_callback 
        # 使用 policy_joint_indices 完成重排序，policy看到yaw在位置12,roll在13,pitch在14
        self.obs[0, 9:24] = self.propri_obs.joint_pos
        # 24:39 - joint_vel (15维)
        joint_vel_scaled = self.propri_obs.joint_vel * 0.05
        self.obs[0, 24:39] = joint_vel_scaled
        # 39:54 - actions (15维)
        self.obs[0, 39:54] = self.actions
        # 54:56 - phase (2维: sin, cos)
        freq = 1.5
        self.phase_t += 0.02*2*np.pi*freq
        self.obs[0, 54] = np.sin(self.phase_t)
        self.obs[0, 55] = np.cos(self.phase_t)
        self.obs[0, 54:56] *= (np.linalg.norm(self.obs[0, 6:9], ord=2) > 0.2)

        # 构建6帧观测 将当前观测数据添加到 obs 的开头
        self.robs = np.concatenate([self.robs.reshape(-1)[56:], self.obs.flatten()]).reshape(1, -1)

        # 执行Policy推理
        if self.policy is not None:
            self.actions = self.policy.run(
                None, 
                {self.policy.get_inputs()[0].name: self.robs}
            )[0][0]
        else:
            self.actions = np.zeros([self.param["policyJointNum"]], dtype=np.float32)

        # 发布电机命令（RUNNING模式：下肢12+腰部3电机policy输出，上肢动作/默认，头部默认）
        self.publish_motor_commands()
    
    def publish_motor_commands(self):
        """发布电机命令（RUNNING模式 - 15关节含腰部）
        
        Policy控制15个电机（12下肢 + 3腰部）:
          - 通过 policy_joint_indices 映射完成重排序
          - policy_joint_indices = [0-11, 14, 12, 13]
          - 即 policy[12]=yaw→motor[14], policy[13]=roll→motor[12], policy[14]=pitch→motor[13]
        ...
        # Policy控制的15个电机（12下肢 + 3腰部，通过policy_joint_indices重排序）
        for i, idx in enumerate(self.policy_joint_indices):
            if i < len(self.actions) and idx < len(self.motor_commands):
                cmd = self.motor_commands[idx]
                cmd.position_des = float(self.actions[i] * 0.25 + self.param["defaultPos"][idx])
        ...
        # 非Policy控制的电机（头部25、手臂15-24）
        # 腰部12-14已由policy通过policy_joint_indices控制，不再需要单独处理
        # 检测是否有运动命令（用于行走摆臂）
        ...
        for i in range(self.param["robotJointNum"]):
            if i not in self.policy_joint_indices:
                ...
                # 行走摆臂：电机15(左肩pitch)和20(右肩pitch)，无手臂动作且有运动命令时
                    ...
                    # 头部(25)、或无动作/无运动命令时的手臂: 使用defaultPos
...
def main(args=None):
    ...
    # 绑定到核心 3-6（核心0-2已被系统通信占用，核心7留给DDS等）
    ...
    # )
    algo_dir_yaml = "/app/etc/algo"  # 或从环境变量读取
    yaml_path = os.path.join(algo_dir_yaml, 'config', 's_waist_15_robot_controller_config.yaml')
    if not os.path.exists(yaml_path):
        yaml_path = os.path.join(
            get_package_share_directory('robot_rl_controller'),
            'config', 's_waist_15_robot_controller_config.yaml',
        )
